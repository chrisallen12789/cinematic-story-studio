...
| VS-419 | Schema-`7.0.0` build evidence embeds validated schema-`8.0.0` Phase 3B results, binds exact app/staged/embedded-service/result/screenshot hashes, proves staged/embedded equality and process/network assertions, excludes private content, and is revalidated immediately before short-lived upload. | Tooling/CI Windows |
...
## Phase 3B.1 governed real local voice activation extension

Phase 0-3B cases remain regression gates. Hosted CI uses only synthetic
metadata and the deterministic fixture provider. The exact ignored Kokoro
package and private generated audio are exercised only by the local packaged
product-path gate and never enter Git or a hosted artifact.

| ID | Required executable behavior | Primary level |
| --- | --- | --- |
| VS-421 | The immutable catalog-v2 inventory preserves all 14 fixture profiles and exposes every technically compatible profile in the exact allow-listed package with neutral labels, exact provider/model/package/tensor identities, sizes, hashes, shapes, provenance, and fail-closed rights classifications. | Pytest/contract/UI |
| VS-422 | Restricted real-voice activation requires the exact displayed warning and a bounded human acknowledgement bound to actor/time, provider, model, package, tensor, voice profile, rights record, catalog, Phase 3A correction and snapshot, runtime, verification, reason, and provenance; stale or mismatched evidence is rejected and no field is promoted to legal, consent, quality, commercial, or production approval. | Pytest/API/UI |
| VS-423 | Assigning a real voice uses the existing Phase 3A immutable correction, lock, restricted-rights, reuse-conflict, and explicit gate-reapproval path. No audition route mutates an assignment, rewrites history, bypasses the current approved cast snapshot, or silently restores approval. | Pytest/Playwright |
| VS-424 | Real sessions resolve provider/runtime per exact assigned role and bind every request, worker, artifact, review, and cache entry to current catalog, rights, activation, assignment, cast snapshot, pronunciation, package, tensor, runtime, and verification evidence. Package absence and catalog, rights, assignment, snapshot, pronunciation, clip, or artifact drift fail closed with targeted invalidation. | Pytest/integration |
| VS-425 | A real per-role decision requires an exact current human-listening attestation cross-bound to clip revision/fingerprint and artifact hash. Fixture or aggregate reviews reject attestations; readiness remains blocked before all required current decisions and never infers listening, naturalness, intelligibility, or quality from automated metrics. | Pytest/API/UI |
| VS-426 | The local packaged product-path gate drives Electron renderer, validated IPC, authenticated service, governed job, exact owned Kokoro worker, 24 kHz mono PCM16 publication, authenticated playback, cache hit, restart restoration, and exact owned-process exit. It creates two narrator-purpose and four character-purpose clips with every eligible profile and records the one-profile inventory limitation without terminating unrelated processes. | Playwright Windows/local real provider |
| VS-427 | The private ignored listening package contains opaque WAV names, a redacted index, neutral voice and role-purpose metadata, exact hashes/duration/format/rights, replay instructions, and a listening scorecard, but no manuscript, audition text, pronunciation value, model/tensor bytes, database, credential, or personal path. | Playwright/repository policy |
| VS-428 | Hosted Windows CI proves absent-package fail-closed behavior and synthetic catalog, rights, acknowledgement, binding, invalidation, UI, schema-`8.0.0` Phase 3B result evidence, artifact exclusion, and retained fixture packaged E2E without downloading, embedding, or uploading a real model, tensor, or generated real audio. | CI/tooling/security |
| VS-429 | Private replay retains state only beneath `%LOCALAPPDATA%\CSS-P3B1\<opaque-id>`, rejects traversal, links and paths above 240 characters, and verifies index/state/app/service hashes before the real launcher starts. From an empty relevant-process baseline it restores the exact project and six clips, handles a live duplicate and stale disk lock markers without broad cleanup, closes only its proven Electron/service tree, and proves every exact owned PID gone. | Vitest/Playwright Windows/local private state |
| VS-430 | Exact historical real per-role reviews accept append-only human decisions only in role chronology with intact activation, clip, artifact, hashes, idempotency, and scope supersession. All outcomes restore identically after restart, while only an exact current acceptable review can contribute to Voice Readiness. | Pytest/Vitest/Playwright Windows |
